...
app.title("CryptVault - Encrypt File")
...
title_label = ctk.CTkLabel(
    main_frame, text="Encrypt Your File", font=("Segoe UI", 27, "bold")
)
...
file_name_label.place(relx=0.5, rely=0.38, anchor="center")
...
def encrypt_selected_file():
    ...
    try:
        ...
        combined_data = filename_bytes + b"||CRYPTVAULT||" + file_data
        ...
        # =====================================
        # ASK USER WHERE TO SAVE .ENC FILE
        # =====================================

        file_name_without_ext = os.path.splitext(os.path.basename(selected_file))[0]

        encrypted_file_path = filedialog.asksaveasfilename(
            title="Save Encrypted File",
            initialfile=file_name_without_ext + ".enc",
        ...
        success_label.configure(
            text=(
                f"✅ {os.path.basename(selected_file)} encrypted successfully\n"
                f"📁 Location: {os.path.basename(os.path.dirname(encrypted_file_path))}"
            ),
            justify="center",
        )
        ...
        file_status_label.configure(text="Choose a file to continue", text_color="gray")
